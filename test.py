...
class AlphaZero:
    ...
    def compare_models(self, initial_model):
        untrained_MCTS = MCTS(self.game, self.args, initial_model)
        trained_MCTS = MCTS(self.game, self.args, self.model)
        ...
        for k in range(2):
            ...
            for i in range(10):
                ...
                state = self.game.get_next_state(state, i // 9, 1)
                player = 1
                while True:
                    ...
                    is_terminal, value = self.game.check_win(state, move)

                    if is_terminal:
                        if MCTS == trained_MCTS and value:
                            trained_score += 1
                        elif MCTS == untrained_MCTS and value:
                            untrained_score += 1
                        elif not value:
                            draw += 1
                        break
    ...
    def learn(self):
        for iteration in range(self.args['num_iterations']):
            ...
            model,win, draw, lose = self.compare_models(initial_model)
            print(win, draw , lose)
